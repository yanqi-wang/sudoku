...
namespace SudokuKata
{
    class Program
    {
        public void Play()
        {
            ...
            PopulateGameBoard(out var rng, out var stateStack, out var rowIndexStack, out var colIndexStack, out var usedDigitsStack, out var lastDigitStack, out Command command, out var board);

            #endregion

            #region Generate inital board from the completely solved one

            CompleteBoard(ref stateStack, ref rng, ref board, out var state, out var finalState);
            #endregion

            #region Prepare lookup structures that will be used in further execution

            PrepareLookupDataStructure(out var maskToOnesCount, out var singleBitToIndex, out var allOnes);
            ...
            while (changeMade)
            {
                ...
                GenerateCandidates(ref state, allOnes, out var candidateMasks);

                #endregion

                #region Build a collection (named cellGroups) which maps cell indices into distinct groups (rows/columns/blocks)

                MapIndicesToBlocks(ref state, out var cellGroups);
                ...
                while (stepChangeMade)
                {
                    ...
                    PickTemp1(ref candidateMasks, maskToOnesCount, ref singleBitToIndex, ref rng, ref state, ref board, ref changeMade);
                    ...
                    if (!changeMade)
                    {
                        CalculateArc3(ref candidateMasks, ref rng, ref state, ref board, ref changeMade);
                    }
                    ...
                    if (!changeMade)
                    {
                        Temp2(candidateMasks, cellGroups, ref rng, ref state, ref board, ref stepChangeMade, maskToOnesCount);
                    }
                    ...
                    {
                        Temp3(candidateMasks, cellGroups, state, ref stepChangeMade, maskToOnesCount);
                    }
                ...
                if (!changeMade)
                {
                    Temp4(ref candidateMasks, maskToOnesCount, ref singleBitToIndex, ref rng,
                        ref state, ref board, ref changeMade, ref command, ref stateStack, finalState, ref rowIndexStack, ref lastDigitStack, ref usedDigitsStack, ref colIndexStack);
                }
        ...
        // can I have a global Random class?
        public void PopulateGameBoard(out Random rng,
            out Stack<int[]> stateStack,
            out Stack<int> rowIndexStack,
            out Stack<int> colIndexStack,
            out Stack<bool[]> usedDigitsStack,
            out Stack<int> lastDigitStack,
            out Command command,
            out char[][] board
        )
        {
            ...
            var boardOperations = new PopulateBoardTemp();
            while (stateStack.Count <= 9 * 9)
            {
                if (command == Command.Expand)
                {
                    command = boardOperations.CommandExpand(rng, stateStack, rowIndexStack, colIndexStack, usedDigitsStack, lastDigitStack, board);
                }
                else if (command == Command.Collapse)
                {
                    command = boardOperations.CommandCollapse(stateStack, rowIndexStack, colIndexStack, usedDigitsStack,
                        lastDigitStack);
                }
                else if (command == Command.Move)
                {
                    command = boardOperations.CommandMove(stateStack, rowIndexStack, colIndexStack, usedDigitsStack, lastDigitStack, board);
                }
            }
        ...
        public void PrepareLookupDataStructure(out Dictionary<int, int> maskToOnesCount, out Dictionary<int, int> singleBitToIndex, out int allOnes)
        {
            Console.WriteLine();
        ...
        public void CompleteBoard(ref Stack<int[]> stateStack, ref Random rng, ref char[][] board, out int[] state, out int[] finalState)
        {
        ...
        public void GenerateCandidates(ref int[] state, int allOnes, out int[] candidateMasks)
        {
            candidateMasks = new int[state.Length];
        ...
        public void MapIndicesToBlocks(ref int[] state, out IEnumerable<IGrouping<int, Cell>> cellGroups)
        {
            var rowsIndices = state
                .Select((value, index) => new Cell()
                {
            ...
            cellGroups = rowsIndices.Concat(columnIndices).Concat(blockIndices).ToList();
        }

        public void PickTemp1(ref int[] candidateMasks, Dictionary<int, int> maskToOnesCount, ref Dictionary<int, int> singleBitToIndex, ref Random rng, ref int[] state, ref char[][] board, ref bool changeMade)
        {
        ...
        public void CalculateArc3(ref int[] candidateMasks, ref Random rng, ref int[] state, ref char[][] board, ref bool changeMade)
        {
        ...
        public void Temp2(int[] candidateMasks, IEnumerable<IGrouping<int, Cell>> cellGroups, ref Random rng, ref int[] state, ref char[][] board, ref bool stepChangeMade, Dictionary<int, int> maskToOnesCount)
        {
        ...
        public void Temp3(int[] candidateMasks, IEnumerable<IGrouping<int, Cell>> cellGroups, int[] state, ref bool stepChangeMade, Dictionary<int, int> maskToOnesCount)
        {
        ...
        public void Temp4(ref int[] candidateMasks, Dictionary<int, int> maskToOnesCount, ref Dictionary<int, int> singleBitToIndex, ref Random rng,
            ref int[] state, ref char[][] board, ref bool changeMade, ref Command command, ref Stack<int[]> stateStack, int[] finalState, ref Stack<int> rowIndexStack, ref Stack<int> lastDigitStack, ref Stack<bool[]> usedDigitsStack, ref Stack<int> colIndexStack)
        {
